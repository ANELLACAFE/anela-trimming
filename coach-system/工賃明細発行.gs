 ...
 *   2. 左の「＋」で新しいスクリプトファイルを追加し、このファイルの内容を貼り付けて保存
 *   3. 下の CONFIG（SAVE_FOLDER_ID・SHOP_NAME）を設定
 *   4. 既存の「コード.gs」の onOpen 内に、下の1行を足す（②の行のすぐ下がおすすめ）:
 *        .addItem('③ 工賃明細を発行（PDF・全員分）', 'issueWageStatements')
 *   5. スプレッドシートを再読み込み → メニュー「工賃システム」→「③ 工賃明細を発行」
 *   6. 対象月を番号で選ぶ → PDFが指定フォルダに保存されます
 *
 *   ※このファイルは onOpen を持ちません。コード.gs にも onOpen があり、
 *     同じ名前の関数が2つあるとメニューが片方しか出ないため、
 *     既存メニュー（工賃システム）に統合する方式にしています。
 * ------------------------------------------------------------------
 */
...
/*
 * ▼ メニューの追加について（重要）
 *   このファイルは onOpen を持ちません。既存の「コード.gs」の onOpen に、
 *   下の1行を足してください（工賃システムのメニューに③として追加されます）:
 *
 *     .addItem('③ 工賃明細を発行（PDF・全員分）', 'issueWageStatements')
 *
 *   例（コード.gs の onOpen）:
 *     .addItem('① knowbeファイルから工賃計算', 'showImportDialog')
